...
function cneDollar(id) { return document.getElementById(id); }
function cneEuro(v) { return Number(v || 0).toLocaleString('pt-PT', { style: 'currency', currency: 'EUR' }); }
function cneEsc(s) { return String(s ?? '').replace(/[&<>"']/g, m => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' }[m])); }
function cneIdFor(prefix, s) {
...
function cneSetSecColor(sec) { document.documentElement.style.setProperty('--sec', CNE_CORES[sec] || CNE_CORES.Agrupamento); }
function cneOfficialMark(sec) {
...
function cneLogoHtml(sec) {
  ...
  return cneOfficialMark(sec);
}

Object.assign(window.CNE, {
  db,
  state: CNE_STATE,
  secoes: CNE_SECOES,
  scopes: CNE_SCOPES,
  cores: CNE_CORES,
  loadConfig: cneLoadConfig,
  loadData: cneLoadData,
  subscribe: cneSubscribe,
  $: CNE.$ || cneDollar,
  euro: CNE.euro || cneEuro,
  esc: CNE.esc || cneEsc,
  idFor: CNE.idFor || cneIdFor,
  setSecColor: CNE.setSecColor || cneSetSecColor,
  logoHtml: CNE.logoHtml || cneLogoHtml,
  officialMark: CNE.officialMark || cneOfficialMark
});